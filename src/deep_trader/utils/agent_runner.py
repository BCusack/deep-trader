...
from deep_trader.agents.trader_agent import create_trader_agent


class AgentRunner:
    def __init__(self):
        self.agent = create_trader_agent()
        self.outputs_dir = Path(__file__).resolve().parents[2] / "outputs"
        self.outputs_dir.mkdir(exist_ok=True)

    def run_agent(self, user_message: str):
        print(f"Running Trader agent with prompt: {user_message}")
        if not user_message:
    ...
    def save_report(self, user_message: str, content: str):
        ...
        output_file = self.outputs_dir / f"trade_plan_{timestamp}.md"
        with output_file.open("w", encoding="utf-8") as file:
            file.write(f"# Crypto Trade Plan\n\n")
